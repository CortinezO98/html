<?php
    ...
    if ($filtro_permanente!="null" AND $filtro_permanente!="") {
        $filtro_buscar="AND (`gcm_nombre_matriz` LIKE ? OR `gcm_estado` LIKE ? OR `gcm_observaciones` LIKE ?)";
    ...
    $consulta_contar_string="SELECT COUNT(`gcm_id`) FROM `tb_gestion_calibracion_matriz` WHERE 1=1 ".$filtro_buscar."";
    ...
    if (!isset($_GET['pagina']) || ($pagina>$numero_paginas AND $numero_paginas>0) || $pagina<=0) {
        header('Location:gestion_calidad_matriz.php?pagina=1&id=null');
    }
    ...
    $consulta_string="SELECT `gcm_id`, `gcm_nombre_matriz`, `gcm_estado`, `gcm_observaciones`, `gcm_registro_usuario`, `gcm_registro_fecha`, TU.`usu_nombres_apellidos` FROM `tb_gestion_calibracion_matriz` LEFT JOIN `tb_administrador_usuario` AS TU ON `tb_gestion_calibracion_matriz`.`gcm_registro_usuario`=TU.`usu_id` WHERE 1=1 ".$filtro_buscar." ORDER BY `gcm_nombre_matriz` LIMIT ?,?";
        ...
        <div class="row" id="elemento_1">
                ...
                <form name="filtrado" action="" method="POST">
                    <div class="input-group">
                      ...
                      <span class="input-group-btn">
                        ...
                        <a href="gestion_calidad_matriz.php?pagina=1&id=null" class="btn btn-corp"><span class="fas fa-sync-alt"></span></a>
                      </span>
                    </div>
                </form>
            </div>
            <div class="col-md-9 py-2">
                <?php if($perfil_modulo=="Administrador" OR $perfil_modulo=="Gestor"): ?>
                    <a href="gestion_calidad_matriz_crear.php?pagina=<?php echo $pagina; ?>&id=<?php echo $filtro_permanente; ?>" class="btn btn-corp menu float-right"><div class="float-left"><span class="fas fa-plus"></span></div><div class="pl-2 menu_res float-left">Crear Matriz</div></a>
                <?php endif; ?>
            </div>
        </div>
                    ...
                    <div class="col-md-12 float-left">
                        <nav aria-label="Paginación" class="paginacion">
                            <ul class="pagination justify-content-end">
                                <li class="page-item <?php echo $pagina<=1 ? 'disabled':'' ?>"><a class="page-link" href="gestion_calidad_matriz.php?pagina=1&id=<?php echo $filtro_permanente; ?>"><span class="fas fa-angle-double-left"></span></a></li>
                                <li class="page-item <?php echo $pagina<=1 ? 'disabled':'' ?>"><a class="page-link" href="gestion_calidad_matriz.php?pagina=<?php echo $pagina-1; ?>&id=<?php echo $filtro_permanente; ?>"><span class="fas fa-angle-left"></span></a></li>
                                <?php
                                ...
                                <?php for ($i=$pagina_inicio; $i <= $pagina_fin; $i++): ?>
                                    <li class="page-item <?php echo $pagina==$i ? 'active':'' ?>"><a class="page-link" href="gestion_calidad_matriz.php?pagina=<?php echo $i; ?>&id=<?php echo $filtro_permanente; ?>"><?php echo $i; ?></a></li>
                                <?php endfor; ?>
                                <li class="page-item <?php echo $pagina>=$numero_paginas ? 'disabled':'' ?>"><a class="page-link" href="gestion_calidad_matriz.php?pagina=<?php echo $pagina+1; ?>&id=<?php echo $filtro_permanente; ?>"><span class="fas fa-angle-right"></a></li>
                                <li class="page-item <?php echo $pagina>=$numero_paginas ? 'disabled':'' ?>"><a class="page-link" href="gestion_calidad_matriz.php?pagina=<?php echo $numero_paginas; ?>&id=<?php echo $filtro_permanente; ?>"><span class="fas fa-angle-double-right"></a></li>
                            </ul>
                        </nav>
                    </div>
                            ...
                            <tbody>    
                                ...
                                <tr>
                                    <td class="align-middle text-center">
                                        <?php if($perfil_modulo=="Administrador" OR $perfil_modulo=="Gestor"): ?>
                                        <a href="gestion_calidad_matriz_editar.php?pagina=<?php echo $pagina; ?>&id=<?php echo $filtro_permanente; ?>&reg=<?php echo base64_encode($resultado_registros[$i][0]); ?>" class="btn btn-primary btn-sm btn-width" title="Editar"><span class="fas fa-pen"></span></a>
                                        <a href="gestion_calidad_matriz_configurar.php?pagina=<?php echo $pagina; ?>&id=<?php echo $filtro_permanente; ?>&reg=<?php echo base64_encode($resultado_registros[$i][0]); ?>" class="btn btn-corp btn-sm btn-width" title="Editar"><span class="fas fa-cog"></span></a>
                                        <?php endif; ?>
                                    </td>
