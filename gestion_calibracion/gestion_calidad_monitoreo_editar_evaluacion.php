<?php
    ...
    if(isset($_POST["actualizar_monitoreo"])){
        ...
        // Prepara la sentencia
        $consulta_actualizar = $enlace_db->prepare("UPDATE `tb_gestion_calibracion_monitoreo` SET `gcm_observaciones_monitoreo`=?, `gcm_nota_enc`=?, `gcm_nota_ecn`=?, `gcm_nota_ecuf`=?, `gcm_nota_general`=?, `gcm_nota_enc_estado`=?, `gcm_nota_ecn_estado`=?, `gcm_nota_ecuf_estado`=? WHERE `gcm_id`=?");
        ...
        if (comprobarSentencia($enlace_db->info)) {
            ...
            for ($i=0; $i < count($items_matriz); $i++) {
                ...
                // Prepara la sentencia
                $sentencia_insert_calificaciones = $enlace_db->prepare("UPDATE `tb_gestion_calibracion_monitoreo_calificaciones` SET `gcmc_respuesta`=?,`gcmc_comentarios`=? WHERE `gcmc_pregunta`=? AND `gcmc_monitoreo`=?");
