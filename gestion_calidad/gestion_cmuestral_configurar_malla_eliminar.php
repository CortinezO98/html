...
// ini_set('display_errors', '1');
    ...
    $titulo_header = "Calculadora Muestral | Configurar - Malla - Eliminar";
    ...
    if(isset($_POST["eliminar_registro"])){
        if($_SESSION['calculadora_malla_calidad_eliminado']!=1){
            // Prepara la sentencia
            $sentencia_delete = $enlace_db->prepare("DELETE FROM `tb_gestion_calidad_cmuestral_malla` WHERE `ccmm_mes`=? AND `ccmm_segmento`=?");
            ...
            if ($sentencia_delete->execute()) {
                // Prepara la sentencia
                $consulta_actualizar = $enlace_db->prepare("UPDATE `tb_gestion_calidad_cmuestral_mensual` SET `cmm_numero_agentes`='0',`cmm_muestras_agente_semana`='0' WHERE `cmm_mes`=? AND `cmm_segmento`=?");

                // Agrega variables a sentencia preparada
                $consulta_actualizar->bind_param('ss', $fecha_calculadora, $id_segmento);
                
                // Ejecuta sentencia preparada
                $consulta_actualizar->execute();
                $_SESSION['calculadora_malla_calidad_eliminado']=1;
    ...
    <div class="contenido">
        ...
        <div class="row" id="tabla_fixed">
            <div class="col-md-12 pt-2">
                ...
                <?php endif; ?>
                <div class="col-md-12">
                    <?php if($_SESSION['calculadora_malla_calidad_eliminado']==1): ?>
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="form-group">
                    <?php if($_SESSION['calculadora_malla_calidad_eliminado']!=1): ?>
                    <button class="btn btn-warning float-right ml-1" type="submit" name="eliminar_registro">Eliminar Malla</button>
                    <?php endif; ?>
                    <?php if($_SESSION['calculadora_malla_calidad_eliminado']==1): ?>
