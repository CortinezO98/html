<?php
    ...
    // Inicializa bandera de doble envío si no existe
    ...
    // ── FIX: validación del consecutivo ──────────────────────────────────────
    // El formato real es "MC" + 10 dígitos (ej: MC0000000147).
    // La versión anterior usaba ctype_digit() que SOLO acepta números puros
    // → bloqueaba TODOS los IDs válidos y redirigía siempre sin mostrar el formulario.
    if ($id_registro === '' || !preg_match('/^[A-Z]{2}\d{10}$/', (string)$id_registro)) {
    ...
    // Si no existe el registro, volver
