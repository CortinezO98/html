...
// ini_set('display_errors', '1');
    ...
    if(isset($_POST["exportar_reporte"])){
        ...
        if ($tipo_reporte=='Monitoreos') {
            ...
            $consulta_string_matriz="SELECT `gcmi_id`, `gcmi_matriz`, `gcmi_item_tipo`, `gcmi_item_consecutivo`, `gcmi_item_orden`, `gcmi_descripcion`, `gcmi_peso`, `gcmi_calificable`, `gcmi_grupo_peso`, `gcmi_visible` FROM `tb_gestion_calidad_matriz_item` WHERE `gcmi_matriz`=? ORDER BY CAST(`gcmi_item_consecutivo` AS UNSIGNED), `gcmi_item_consecutivo` ASC, CONVERT(`gcmi_item_orden`, SIGNED INTEGER)";
            ...
            $consulta_string_matriz_nombre="SELECT `gcm_id`, `gcm_nombre_matriz`, `gcm_estado`, `gcm_observaciones`, `gcm_registro_usuario`, `gcm_registro_fecha` FROM `tb_gestion_calidad_matriz` WHERE `gcm_id`=?";
            ...
            for ($i=4; $i < count($resultado_registros)+4; $i++) {
                ...
                $consulta_string_historial="SELECT `gcmh_id`, `gcmh_monitoreo`, `gcmh_tipo_cambio`, `gcmh_comentarios`, `gcmh_registro_usuario`, `gcmh_registro_fecha` FROM `tb_gestion_calidad_monitoreo_historial` WHERE `gcmh_monitoreo`=?";
                ...
                $consulta_string_respuesta="SELECT `gcmc_id`, `gcmc_monitoreo`, `gcmc_pregunta`, `gcmc_respuesta`, `gcmc_afectaciones`, `gcmc_comentarios`, TIM.`gcmi_matriz`, TIM.`gcmi_item_tipo`, TIM.`gcmi_item_consecutivo`, TIM.`gcmi_item_orden`, TIM.`gcmi_descripcion`, TIM.`gcmi_peso`, TIM.`gcmi_calificable` FROM `tb_gestion_calidad_monitoreo_calificaciones` LEFT JOIN `tb_gestion_calidad_matriz_item` AS TIM ON `tb_gestion_calidad_monitoreo_calificaciones`.`gcmc_pregunta`=TIM.`gcmi_id` WHERE `gcmc_monitoreo`=?  ORDER BY TIM.`gcmi_item_consecutivo` ASC";
            ...
            $consulta_string_soportes="SELECT `gcmhs_id`, `gcmhs_monitoreo`, `gcmhs_historial`, `gcmhs_nombre`, `gcmhs_ruta`, `gcmhs_extension`, `gcmhs_registro_usuario`, `gcmhs_registro_fecha` FROM `tb_gestion_calidad_monitoreo_historial_soportes` WHERE `gcmhs_registro_fecha`>=? AND `gcmhs_registro_fecha`<=?";
