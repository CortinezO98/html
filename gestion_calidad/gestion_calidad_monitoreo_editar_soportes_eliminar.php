<?php
    ...
    if(isset($_POST["eliminar_registro"])){
        if($_SESSION['monitoreo_registro_eliminado_doc']!=1){
            if ($tipo=="mon") {
                $consulta_string_soportes="SELECT `gcms_id`, `gcms_monitoreo`, `gcms_nombre`, `gcms_ruta`, `gcms_extension`, `gcms_registro_usuario`, `gcms_registro_fecha` FROM `tb_gestion_calidad_monitoreo_soportes` WHERE `gcms_id`=?";
            } elseif ($tipo=="his") {
                $consulta_string_soportes="SELECT `gcmhs_id`, `gcmhs_monitoreo`, `gcmhs_nombre`, `gcmhs_ruta`, `gcmhs_extension`, `gcmhs_registro_usuario`, `gcmhs_registro_fecha` FROM `tb_gestion_calidad_monitoreo_historial_soportes` WHERE `gcmhs_id`=?";
            }
            ...
            if ($tipo=="mon") {
                ...
                $sentencia_delete_soportes = $enlace_db->prepare("DELETE FROM `tb_gestion_calidad_monitoreo_soportes` WHERE `gcms_id`=?");
            } elseif ($tipo=="his") {
                // Prepara la sentencia
                $sentencia_delete_soportes = $enlace_db->prepare("DELETE FROM `tb_gestion_calidad_monitoreo_historial_soportes` WHERE `gcmhs_id`=?");
            }
