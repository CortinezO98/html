...
// ini_set('display_errors', '1');
    ...
    $titulo_header = "Calculadora Muestral | Configuración - Malla";
    ...
    if(isset($_POST["guardar_registro"])){
        ...
        $sentencia_insert = $enlace_db->prepare("INSERT INTO `tb_gestion_calidad_cmuestral_malla`(`ccmm_id`, `ccmm_mes`, `ccmm_segmento`, `ccmm_usuario`, `ccmm_usuario_segmento`, `ccmm_coordinador`, `ccmm_lunes`, `ccmm_martes`, `ccmm_miercoles`, `ccmm_jueves`, `ccmm_viernes`, `ccmm_sabado`, `ccmm_domingo`, `ccmm_monitoreo_lunes`, `ccmm_monitoreo_martes`, `ccmm_monitoreo_miercoles`, `ccmm_monitoreo_jueves`, `ccmm_monitoreo_viernes`, `ccmm_monitoreo_sabado`, `ccmm_monitoreo_domingo`, `ccmm_actualiza_usuario`, `ccmm_actualiza_fecha`, `ccmm_monitor`, `ccmm_registro_usuario`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");

        // Agrega variables a sentencia preparada
        $sentencia_insert->bind_param('ssssssssssssssssssssssss', $id_registro_turno, $fecha_calculadora_registro, $id_segmento, $registro_usuario, $registro_usuario_segmento, $registro_coordinador, $registro_turno_lunes, $registro_turno_martes, $registro_turno_miercoles, $registro_turno_jueves, $registro_turno_viernes, $registro_turno_sabado, $registro_turno_domingo, $registro_turno_mlunes, $registro_turno_mmartes, $registro_turno_mmiercoles, $registro_turno_mjueves, $registro_turno_mviernes, $registro_turno_msabado, $registro_turno_mdomingo, $ccmm_actualiza_usuario, $ccmm_actualiza_fecha, $registro_turno_monitor, $ccmm_registro_usuario);
        $control_insert=0;
        $control_insert_malla=0;
        $control_registros=0;
        ...
        for ($i=0; $i < count($documento_identidad); $i++) { 
            ...
            $registro_usuario_segmento=$usuarios_detalle[$registro_usuario]['segmento'];
            $registro_coordinador=$usuarios_detalle[$registro_usuario]['supervisor'];
            ...
            $registro_turno_mlunes='';
            $registro_turno_mmartes='';
            $registro_turno_mmiercoles='';
            $registro_turno_mjueves='';
            $registro_turno_mviernes='';
            $registro_turno_msabado='';
            $registro_turno_mdomingo='';
            $ccmm_actualiza_usuario=$_SESSION["usu_id"];
            $ccmm_actualiza_fecha=date('Y-m-d H:i:s');
            $registro_turno_monitor='';
            $ccmm_registro_usuario=$_SESSION["usu_id"];

            if ($sentencia_insert->execute()) {
                $control_insert++;

                if (($array_turno_programado[$registro_usuario]['turno'][1]!="" AND strpos($array_turno_programado[$registro_usuario]['turno'][1], '-')!="") OR ($array_turno_programado[$registro_usuario]['turno'][2]!="" AND strpos($array_turno_programado[$registro_usuario]['turno'][2], '-')!="") OR ($array_turno_programado[$registro_usuario]['turno'][3]!="" AND strpos($array_turno_programado[$registro_usuario]['turno'][3], '-')!="") OR ($array_turno_programado[$registro_usuario]['turno'][4]!="" AND strpos($array_turno_programado[$registro_usuario]['turno'][4], '-')!="") OR ($array_turno_programado[$registro_usuario]['turno'][5]!="" AND strpos($array_turno_programado[$registro_usuario]['turno'][5], '-')!="") OR ($array_turno_programado[$registro_usuario]['turno'][6]!="" AND strpos($array_turno_programado[$registro_usuario]['turno'][6], '-')!="") OR ($array_turno_programado[$registro_usuario]['turno'][0]!="" AND strpos($array_turno_programado[$registro_usuario]['turno'][0], '-')!="")) {
                    $control_insert_malla++;
                }
        ...
        if ($control_insert==$control_registros) {
            ...
            $_SESSION['registro_cargue_malla']=1;
            $cmm_numero_agentes=$control_insert_malla;
            $cmm_muestras_agente_semana_temp=$resultado_registros_fechas[0][18]/$cmm_numero_agentes;
            $cmm_muestras_agente_semana=round($resultado_registros_fechas[0][18]/$cmm_numero_agentes);
            if (($cmm_muestras_agente_semana_temp-$cmm_muestras_agente_semana)>0.5) {
                $cmm_muestras_agente_semana+=1;
            }
            // Prepara la sentencia
            $consulta_actualizar = $enlace_db->prepare("UPDATE `tb_gestion_calidad_cmuestral_mensual` SET `cmm_numero_agentes`=?,`cmm_muestras_agente_semana`=? WHERE `cmm_calculadora`=? AND `cmm_mes`=? AND `cmm_segmento`=?");

            // Agrega variables a sentencia preparada
            $consulta_actualizar->bind_param('sssss', $cmm_numero_agentes, $cmm_muestras_agente_semana, $id_registro, $fecha_calculadora, $id_segmento);
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-12">
                            <div class="form-group">
                              ...
                              <textarea class="form-control form-control-sm" name="documento_identidad" id="documento_identidad" <?php if($_SESSION['registro_cargue_malla']==1) { echo 'disabled'; } ?> required rows="10"><?php if(isset($_POST["guardar_registro"])){ echo $documento_identidad_pre; } ?></textarea>
                            </div>
                        </div>
                        <div class="col-md-12">
                            <div class="form-group">
                                <?php if($_SESSION['registro_cargue_malla']==1): ?>
