...
$bandeja_raw = isset($_GET['bandeja']) ? base64_decode($_GET['bandeja']) : '';
$bandeja = validar_input((string)$bandeja_raw);

// Evitar warning si no existe la bandera
if (!isset($_SESSION['gcmta_registro_creado'])) {
    $_SESSION['gcmta_registro_creado'] = 0;
}

// ✅ Auditor responsable (en tu BD NO NULL -> si no existe, queda '')
...
if (isset($_POST["guardar_registro"])) {
    ...
    $telefonico = (int)validar_input($_POST['telefonico'] ?? '0');
    $virtual = (int)validar_input($_POST['virtual'] ?? '0');

    if ($_SESSION['gcmta_registro_creado'] != 1) {

        // 1) Validar si ya existen auditorías en ese rango
        $consulta_string_auditoria_count = "
            SELECT COUNT(`tb_gestion_calidad_monitoreo`.`gcm_id`)
            FROM `tb_gestion_calidad_monitoreo`
            LEFT JOIN `tb_gestion_calidad_matriz` AS TM
                ON `tb_gestion_calidad_monitoreo`.`gcm_matriz` = TM.`gcm_id`
            WHERE `tb_gestion_calidad_monitoreo`.`gcm_auditoria` = 'Si'
              AND `tb_gestion_calidad_monitoreo`.`gcm_registro_fecha` >= ?
              AND `tb_gestion_calidad_monitoreo`.`gcm_registro_fecha` <= ?
        ";
        ...
        $total_auditorias = (int)($resultado_registros_auditoria_count[0][0] ?? 0);

        if ($total_auditorias === 0) {

            // ✅ UPDATE recomendado: marca auditoría y asigna responsable SOLO si viene vacío
            $consulta_actualizar_auditoria = $enlace_db->prepare("
                UPDATE `tb_gestion_calidad_monitoreo`
                SET
                    `gcm_auditoria` = ?,
                    `gcm_auditoria_responsable` = IF(
                        `gcm_auditoria_responsable` = '' OR `gcm_auditoria_responsable` IS NULL,
                        ?,
            ...
            $consulta_string_auditoria_telefonico = "
                SELECT
                    ...
                    TMON.`gcm_matriz`,
                    TMON.`gcm_analista`,
                    CAST(TMON.`gcm_nota_general` AS UNSIGNED)
                FROM `tb_gestion_calidad_monitoreo` AS TMON
                WHERE TMON.`gcm_aplica_indicador` = 'Si'
                  AND TMON.`gcm_registro_fecha` >= ?
                  AND TMON.`gcm_registro_fecha` <= ?
                  AND CAST(TMON.`gcm_nota_general` AS UNSIGNED) >= 91
                  AND (
                        TMON.`gcm_segmento` = 'Línea 141'
                     OR TMON.`gcm_segmento` = 'Línea Nacional'
                     OR TMON.`gcm_segmento` = 'Conmutador'
                  )
                  AND TMON.`gcm_fecha_hora_cierre` > 3
                  AND TMON.`gcm_fecha_hora_cierre` <= 30
                ORDER BY CAST(TMON.`gcm_nota_general` AS UNSIGNED) ASC
            ";
            ...
            $array_agentes = [];
            $array_agentes_monitoreos = [];

            for ($i = 0; $i < count($resultado_registros_auditoria_telefonico); $i++) {
                $agente = (string)$resultado_registros_auditoria_telefonico[$i][2];
                $mon_id = (string)$resultado_registros_auditoria_telefonico[$i][0];

                $array_agentes[] = $agente;
                $array_agentes_monitoreos[$agente][] = $mon_id;

                shuffle($array_agentes_monitoreos[$agente]);
            }
            ...
            // Evitar warnings si piden más de los que hay
            ...
            for ($j = 0; $j < $telefonico; $j++) {
                ...
                // si se pasa del tamaño, rotamos
                if ($j >= count($array_agentes)) { $j = 0; }

                $id_agente = $array_agentes[$j];

                if (!isset($array_agentes_monitoreos[$id_agente][0])) {
                    continue;
                }

                $gcm_id = (string)$array_agentes_monitoreos[$id_agente][0];

                if ($gcm_id !== '') {
                    $gcm_auditoria = 'Si';
                    $gcm_auditoria_responsable = $auditor_user; // ✅ set solo si está vacío
            ...
            $consulta_string_auditoria_virtual = "
                ...
                WHERE TMON.`gcm_aplica_indicador` = 'Si'
                  ...
                  AND CAST(TMON.`gcm_nota_general` AS UNSIGNED) >= 91
                  AND (
                        TMON.`gcm_segmento`='Aseguramiento'
                     OR TMON.`gcm_segmento`='Encuestas'
                     OR TMON.`gcm_segmento`='Profesional Es Abogados'
                     OR TMON.`gcm_segmento`='Profesional ES Psicólogos'
                     OR TMON.`gcm_segmento`='WhatsApp'
                     OR TMON.`gcm_segmento`='Video Llamada'
                     OR TMON.`gcm_segmento`='Correos y Portales'
                     OR TMON.`gcm_segmento`='Presencial'
                     OR TMON.`gcm_segmento`='Chat ICBF'
                     OR TMON.`gcm_segmento`='Canal escrito'
                     OR TMON.`gcm_segmento`='Redes Sociales'
                  )
                ORDER BY CAST(TMON.`gcm_nota_general` AS UNSIGNED) ASC
            ";
            ...
            for ($i = 0; $i < count($resultado_registros_auditoria_virtual); $i++) {
                $agente = (string)$resultado_registros_auditoria_virtual[$i][2];
                $mon_id = (string)$resultado_registros_auditoria_virtual[$i][0];
...
$consulta_string_matriz = "
    ...
    ORDER BY `gcm_nombre_matriz` ASC
";
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="date"
                                   ...
                                   name="fecha_inicio"
                                   id="fecha_inicio"
                                   value="<?php if(isset($_POST["guardar_registro"])) { echo $fecha_inicio; } ?>"
                                   <?php if($_SESSION['gcmta_registro_creado']==1) { echo 'readonly'; } ?>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="date"
                                   ...
                                   name="fecha_fin"
                                   id="fecha_fin"
                                   value="<?php if(isset($_POST["guardar_registro"])) { echo $fecha_fin; } ?>"
                                   <?php if($_SESSION['gcmta_registro_creado']==1) { echo 'readonly'; } ?>
                                   required autocomplete="off">
                        </div>
                    </div>
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            <input type="number"
                                   ...
                                   id="telefonico"
                                   min="1" max="500"
                                   value="<?php if(isset($_POST["guardar_registro"])) { echo (int)$telefonico; } ?>"
                                   <?php if($_SESSION['gcmta_registro_creado']==1) { echo 'readonly'; } ?>
                                   required>
                        </div>
                    </div>
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            <input type="number"
                                   ...
                                   id="virtual"
                                   min="1" max="500"
                                   value="<?php if(isset($_POST["guardar_registro"])) { echo (int)$virtual; } ?>"
                                   <?php if($_SESSION['gcmta_registro_creado']==1) { echo 'readonly'; } ?>
                                   required>
                        </div>
                    </div>
