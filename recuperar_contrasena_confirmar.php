    ...
    if(isset($_SESSION["usu_id"]) AND $_SESSION['usu_inicio_sesion']!=0){
    	...
    	if(isset($_POST["form_recovery_confirm"])){
            ...
            if (!isset($_SESSION["password_recovery"])) {
	            $consulta_string = "SELECT `tk_usuario`, `tk_token`, `tk_estado` FROM `tb_administrador_token` WHERE `tk_usuario`=? AND `tk_token`=? AND TIME_TO_SEC(TIMEDIFF(NOW(), `tk_registro_fecha`))<'180'";
