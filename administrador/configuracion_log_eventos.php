<?php
    ...
    $titulo_header = "Administrador | Log de Eventos";
    ...
    if ($filtro_permanente!="null" AND $filtro_permanente!="") {
        $filtro_buscar="AND (`clog_log_modulo` LIKE ? OR `clog_log_tipo` LIKE ? OR `clog_log_accion` LIKE ? OR `clog_log_detalle` LIKE ? OR `clog_registro_usuario` LIKE ? OR `clog_registro_fecha` LIKE ? OR TU.`usu_nombres_apellidos` LIKE ?)";
    ...
    $consulta_contar_string="SELECT COUNT(`clog_id`) FROM `tb_administrador_log` LEFT JOIN `tb_administrador_usuario` AS TU ON `tb_administrador_log`.`clog_registro_usuario`=TU.`usu_id` WHERE 1=1 ".$filtro_buscar."";
    ...
    if (!isset($_GET['pagina']) || ($pagina>$numero_paginas AND $numero_paginas>0) || $pagina<=0) {
        header('Location:configuracion_log_eventos.php?pagina=1&id=null');
    }
    ...
    $consulta_string="SELECT `clog_id`, `clog_log_modulo`, `clog_log_tipo`, `clog_log_accion`, `clog_log_detalle`, `clog_registro_usuario`, `clog_registro_fecha`, TU.`usu_nombres_apellidos` FROM `tb_administrador_log` LEFT JOIN `tb_administrador_usuario` AS TU ON `tb_administrador_log`.`clog_registro_usuario`=TU.`usu_id` WHERE 1=1 ".$filtro_buscar." ORDER BY `clog_registro_fecha` DESC LIMIT ?,?";
                ...
                <form name="filtrado" action="" method="POST">
                    <div class="input-group">
                      ...
                      <span class="input-group-btn">
                        ...
                        <a href="configuracion_log_eventos.php?pagina=1&id=null" class="btn btn-corp"><span class="fas fa-sync-alt"></span></a>
                      </span>
                    </div>
                </form>
            </div>
                    ...
                    <div class="col-md-12 float-left">
                        <nav aria-label="Paginación" class="paginacion">
                            <ul class="pagination justify-content-end">
                                <li class="page-item <?php echo $pagina<=1 ? 'disabled':'' ?>"><a class="page-link" href="configuracion_log_eventos.php?pagina=1&id=<?php echo $filtro_permanente; ?>"><span class="fas fa-angle-double-left"></span></a></li>
                                <li class="page-item <?php echo $pagina<=1 ? 'disabled':'' ?>"><a class="page-link" href="configuracion_log_eventos.php?pagina=<?php echo $pagina-1; ?>&id=<?php echo $filtro_permanente; ?>"><span class="fas fa-angle-left"></span></a></li>
                                <?php
                                ...
                                <?php for ($i=$pagina_inicio; $i <= $pagina_fin; $i++): ?>
                                    <li class="page-item <?php echo $pagina==$i ? 'active':'' ?>"><a class="page-link" href="configuracion_log_eventos.php?pagina=<?php echo $i; ?>&id=<?php echo $filtro_permanente; ?>"><?php echo $i; ?></a></li>
                                <?php endfor; ?>
                                <li class="page-item <?php echo $pagina>=$numero_paginas ? 'disabled':'' ?>"><a class="page-link" href="configuracion_log_eventos.php?pagina=<?php echo $pagina+1; ?>&id=<?php echo $filtro_permanente; ?>"><span class="fas fa-angle-right"></a></li>
                                <li class="page-item <?php echo $pagina>=$numero_paginas ? 'disabled':'' ?>"><a class="page-link" href="configuracion_log_eventos.php?pagina=<?php echo $numero_paginas; ?>&id=<?php echo $filtro_permanente; ?>"><span class="fas fa-angle-double-right"></a></li>
                            </ul>
                        </nav>
                    </div>
                        ...
                        <table class="table table-bordered table-striped table-hover table-sm">
                            <thead>
                                <tr>
                                    ...
                                    <th class="align-middle" style="min-width: 115px;">Fecha y Hora</th>
                                    <th class="align-middle">Módulo</th>
                                    <th class="align-middle" style="min-width: 95px;">Acción</th>
                            ...
                            <tbody>    
                                ...
                                <tr>
                                    <td class="align-middle text-center">
                                        <?php if ($resultado_registros[$i][2]=="inicio_sesion"): ?>
                                            <span class="fas fa-sign-in-alt"></span>
                                        <?php elseif($resultado_registros[$i][2]=="crear"): ?>
                                            <span class="fas fa-plus-square"></span>
                                        <?php elseif($resultado_registros[$i][2]=="editar"): ?>
                                            <span class="fas fa-edit"></span>
                                        <?php elseif($resultado_registros[$i][2]=="eliminar"): ?>
                                            <span class="fas fa-minus-square"></span>
                                        <?php elseif($resultado_registros[$i][2]=="notificacion"): ?>
                                            <span class="fas fa-envelope"></span>
                                        <?php elseif($resultado_registros[$i][2]=="notificacion_error"): ?>
