...
// ini_set('display_errors', '1');
    ...
    unset($_SESSION['monitoreo_creado_auditoria']);
    ...
    if($bandeja=="Mes Actual"){
        $filtro_bandeja=" AND TMC.`gcm_estado`<>? AND TMC.`gcm_estado`<>? AND TMC.`gcm_registro_fecha` LIKE ?";
        array_push($data_consulta, 'Refutado');
        array_push($data_consulta, 'Refutado-Conciliación');
        $mes_actual=date('Y-m');
        ...
        $filtro_bandeja=" AND TMC.`gcm_estado`=?";
        array_push($data_consulta, 'Refutado');
    } elseif($bandeja=="Refutado-Conciliación"){
        $filtro_bandeja=" AND TMC.`gcm_estado`=?";
        array_push($data_consulta, 'Refutado-Conciliación');
    ...
    $consulta_contar_string="SELECT COUNT(TMC.`gcm_id`) FROM `tb_gestion_auditoria_monitoreo` AS TMC 
    ...
    LEFT JOIN `tb_gestion_calidad_monitoreo` AS TMO ON TMC.`gcm_id`=TMO.`gcm_id` WHERE 1=1 ".$filtro_buscar." ".$filtro_perfil." ".$filtro_bandeja."";
    ...
    $consulta_string="SELECT TMC.`gcm_id`, TM.`gcm_nombre_matriz`, TUA.`usu_nombres_apellidos`, TMC.`gcm_fecha_hora_gestion`, TMC.`gcm_fecha_hora_cierre`, TMC.`gcm_tipo_monitoreo`, TMC.`gcm_skill_interaccion`, TMC.`gcm_tipo_gestion`, TMC.`gcm_segmento`, TMC.`gcm_id_sim`, TMC.`gcm_id_ani`, TMC.`gcm_observaciones_monitoreo`, TMC.`gcm_nota_enc`, TMC.`gcm_nota_ecn`, TMC.`gcm_nota_ecuf`, TMC.`gcm_estado`, TUR.`usu_nombres_apellidos`, TMC.`gcm_registro_fecha`, TP.`ap_nombre_piloto`, TS.`usu_nombres_apellidos`, TMC.`gcm_nota_general`, TMC.`gcm_nota_enc_estado`, TMC.`gcm_nota_ecn_estado`, TMC.`gcm_nota_ecuf_estado`, TMC.`gcm_afectacion_1`, TMC.`gcm_fecha_monitoreo`, TMC.`gcm_registro_usuario`, TM.`gcm_observaciones`, TSA.`usu_nombres_apellidos` FROM `tb_gestion_auditoria_monitoreo` AS TMC 
        ...
        LEFT JOIN `tb_administrador_usuario` AS TS ON TMC.`gcm_responsable`=TS.`usu_id` 
        LEFT JOIN `tb_administrador_usuario` AS TSA ON TUA.`usu_supervisor`=TSA.`usu_id`
        LEFT JOIN `tb_gestion_calidad_monitoreo` AS TMO ON TMC.`gcm_id`=TMO.`gcm_id`  WHERE 1=1 ".$filtro_buscar." ".$filtro_perfil." ".$filtro_bandeja." ORDER BY `gcm_id` DESC LIMIT ?,?";
    ...
    <div class="contenido">
        <div class="row" id="elemento_1">
            ...
            <div class="col-md-9 py-2">
                <?php if($perfil_modulo=="Administrador" OR $perfil_modulo=="Gestor" OR $perfil_modulo=="Supervisor"): ?>
                ...
                <a href="gestion_calidad_monitoreo.php?pagina=1&id=<?php echo $filtro_permanente; ?>&bandeja=<?php echo base64_encode('Refutado-Conciliación'); ?>" class="btn btn-corp menu float-right"><div class="float-left"><span class="fas fa-user-times"></span></div><div class="pl-2 menu_res float-left">Refutados-Conciliación</div></a>
                ...
                <a href="gestion_calidad_monitoreo.php?pagina=1&id=<?php echo $filtro_permanente; ?>&bandeja=<?php echo base64_encode('Pendientes'); ?>" class="btn btn-corp menu float-right"><div class="float-left"><span class="fas fa-user-clock"></span></div><div class="pl-2 menu_res float-left">Pendientes</div></a>
                ...
                <?php if($perfil_modulo=="Administrador" OR $perfil_modulo=="Usuario"): ?>
                        ...
                        <table class="table table-bordered table-striped table-hover table-sm">
                            <thead>
                                <tr>
                                    ...
                                    <th class="align-middle">Fecha Auditoría</th>
                                </tr>
                            </thead>
                            <tbody>    
                                <?php
                                ...
                                <tr>
                                    <td class="align-middle text-center">
                                        <?php if ($perfil_modulo=='Usuario' AND $resultado_registros[$i][15]=="Pendiente"): ?>
                                        ...
                                        <?php if ($perfil_modulo=='Usuario' AND $resultado_registros[$i][15]=='Refutado-Rechazado'): ?>
                                            <a href="gestion_calidad_monitoreo_aceptar.php?pagina=<?php echo $pagina; ?>&id=<?php echo $filtro_permanente; ?>&reg=<?php echo base64_encode($resultado_registros[$i][0]); ?>&bandeja=<?php echo base64_encode($bandeja); ?>" class="btn btn-corp btn-sm btn-width mb-1" title="Aceptar"><span class="fas fa-check-circle"></span></a>
                                            <a href="gestion_calidad_monitoreo_refutar.php?pagina=<?php echo $pagina; ?>&id=<?php echo $filtro_permanente; ?>&reg=<?php echo base64_encode($resultado_registros[$i][0]); ?>&bandeja=<?php echo base64_encode($bandeja); ?>&calidad=Conciliar" class="btn btn-danger btn-sm btn-width mb-1" title="Conciliar"><span class="fas fa-times-circle"></span></a>
                                        <?php endif; ?>

                                        <?php if ($perfil_modulo=='Administrador' AND $resultado_registros[$i][15]=='Refutado'): ?>
                                        ...
                                        <?php if ($perfil_modulo=='Usuario' AND $resultado_registros[$i][15]=='Refutado-Conciliación'): ?>
                                            <a href="gestion_calidad_monitoreo_conciliar.php?pagina=<?php echo $pagina; ?>&id=<?php echo $filtro_permanente; ?>&reg=<?php echo base64_encode($resultado_registros[$i][0]); ?>&bandeja=<?php echo base64_encode($bandeja); ?>" class="btn btn-warning btn-sm btn-width mb-1" title="Conciliar"><span class="fas fa-lock"></span></a>
                                        <?php endif; ?>
                                    ...
                                    <td class="align-middle text-center <?php if($resultado_registros[$i][15]=="Refutado" OR $resultado_registros[$i][15]=="Refutado-Conciliación" OR $resultado_registros[$i][15]=="Refutado-Rechazado") {echo "rechazado";} elseif($resultado_registros[$i][15]=="Aceptado" OR $resultado_registros[$i][15]=="Refutado-Aceptado" OR $resultado_registros[$i][15]=="Conciliado Cliente" OR $resultado_registros[$i][15]=="Conciliado Proveedor") {echo "aceptado";} ?>">
                                        ...
                                        <?php if($resultado_registros[$i][15]=="Refutado-Conciliación") {echo "<span class='fas fa-user-times'></span>";} ?>
                                        ...
                                        <?php if($resultado_registros[$i][15]=="Conciliado Cliente" OR $resultado_registros[$i][15]=="Conciliado Proveedor") {echo "<span class='fas fa-user-lock'></span>";} ?>
