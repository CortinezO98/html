<?php
    ...
    $consulta_string_adjuntos_monitoreo="SELECT `gcms_id`, `gcms_monitoreo`, `gcms_nombre`, `gcms_ruta`, `gcms_extension`, `gcms_registro_usuario`, `gcms_registro_fecha` FROM `tb_gestion_calidad_monitoreo_soportes` WHERE `gcms_monitoreo`=?";

    $consulta_registros_adjuntos_monitoreo = $enlace_db->prepare($consulta_string_adjuntos_monitoreo);
    $consulta_registros_adjuntos_monitoreo->bind_param("s", $id_registro);
    $consulta_registros_adjuntos_monitoreo->execute();
    $resultado_registros_adjuntos_monitoreo = $consulta_registros_adjuntos_monitoreo->get_result()->fetch_all(MYSQLI_NUM);
    ...
    $consulta_string_historial="SELECT `gcmh_id`, `gcmh_monitoreo`, `gcmh_tipo_cambio`, `gcmh_comentarios`, `gcmh_registro_usuario`, `gcmh_registro_fecha`, TUR.`usu_nombres_apellidos` FROM `tb_gestion_auditoria_monitoreo_historial` LEFT JOIN `tb_administrador_usuario` AS TUR ON `tb_gestion_auditoria_monitoreo_historial`.`gcmh_registro_usuario`=TUR.`usu_id` WHERE `gcmh_monitoreo`=? ORDER BY `gcmh_registro_fecha` DESC";
...
<div class="row">
    <div class="col-md-3">
        ...
        <div class="font-size-11 p-1">
            <?php if(isset($resultado_registros_adjuntos_monitoreo)): ?>
                <?php for ($i=0; $i < count($resultado_registros_adjuntos_monitoreo); $i++): ?>
                    <a href="gestion_calidad_monitoreo_ver_soporte_descargar.php?reg=<?php echo $resultado_registros_adjuntos_monitoreo[$i][0]; ?>&tipo=monitoreo" target="_blank" title="Descargar"><span class="fas fa-download"></span></a> <a href="../gestion_calidad/<?php echo $resultado_registros_adjuntos_monitoreo[$i][3]; ?>" target="_blank" title="<?php echo $resultado_registros_adjuntos_monitoreo[$i][2]; ?>"><span class="fas fa-external-link-alt"></span></a> <?php echo (strlen($resultado_registros_adjuntos_monitoreo[$i][2])>40)? substr($resultado_registros_adjuntos_monitoreo[$i][2], 0, 40).'...' : $resultado_registros_adjuntos_monitoreo[$i][2]; ?><br>
                <?php endfor; ?>
            ...
            <?php endif; ?>
            <?php if(isset($resultado_registros_adjuntos)): ?>
                ...
                <embed id="embed1" src="" width="100%" height="500px" class="d-none mt-0 visor_div" type="application/pdf"/>
            <?php endif; ?>
            <?php if(!isset($resultado_registros_adjuntos_monitoreo) AND !isset($resultado_registros_adjuntos)): ?>
                <p class="alert alert-warning p-1">
                    <span class="fas fa-exclamation-triangle"></span> No se encontraron registros
                </p>
            <?php endif; ?>
            <?php if(isset($resultado_registros_adjuntos_his)): ?>
                <?php for ($i=0; $i < count($resultado_registros_adjuntos_his); $i++): ?>
                    <a href="gestion_calidad_monitoreo_ver_soporte_descargar.php?reg=<?php echo $resultado_registros_adjuntos_his[$i][0]; ?>&tipo=his" target="_blank" title="Descargar"><span class="fas fa-download"></span></a> <a href="<?php echo $resultado_registros_adjuntos_his[$i][3]; ?>" target="_blank" title="<?php echo $resultado_registros_adjuntos_his[$i][2]; ?>"><span class="fas fa-external-link-alt"></span></a> <?php echo (strlen($resultado_registros_adjuntos_his[$i][2])>40)? substr($resultado_registros_adjuntos_his[$i][2], 0, 40).'...' : $resultado_registros_adjuntos_his[$i][2]; ?><br>
                <?php endfor; ?>
                <p class="alert alert-warning p-1 d-none mb-0" id="titulo_visor"></p>                
                <embed id="embed1" src="" width="100%" height="500px" class="d-none mt-0 visor_div" type="application/pdf"/>
            <?php endif; ?>
