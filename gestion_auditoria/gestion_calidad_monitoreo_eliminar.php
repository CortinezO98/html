<?php
    ...
    if(isset($_POST["eliminar_registro"])){
        if($_SESSION['monitoreo_registro_eliminado']!=1){
            ...
            $sentencia_delete_historial = $enlace_db->prepare("DELETE FROM `tb_gestion_auditoria_monitoreo_historial` WHERE `gcmh_monitoreo`=?");
            // Agrega variables a sentencia preparada
            $sentencia_delete_historial->bind_param('s', $id_registro);

            $consulta_string_soportes="SELECT `gcms_id`, `gcms_monitoreo`, `gcms_nombre`, `gcms_ruta`, `gcms_extension`, `gcms_registro_usuario`, `gcms_registro_fecha` FROM `tb_gestion_auditoria_monitoreo_soportes` WHERE `gcms_monitoreo`=?";
            ...
            $consulta_string_soportes_his="SELECT `gcmhs_id`, `gcmhs_monitoreo`, `gcmhs_nombre`, `gcmhs_ruta`, `gcmhs_extension`, `gcmhs_registro_usuario`, `gcmhs_registro_fecha` FROM `tb_gestion_auditoria_monitoreo_historial_soportes` WHERE `gcmhs_monitoreo`=?";
            ...
            // Prepara la sentencia
            $sentencia_delete_soportes = $enlace_db->prepare("DELETE FROM `tb_gestion_auditoria_monitoreo_soportes` WHERE `gcms_monitoreo`=?");
            // Agrega variables a sentencia preparada
            $sentencia_delete_soportes->bind_param('s', $id_registro);

            // Prepara la sentencia
            $sentencia_delete_soportes_his = $enlace_db->prepare("DELETE FROM `tb_gestion_auditoria_monitoreo_historial_soportes` WHERE `gcmhs_monitoreo`=?");
            // Agrega variables a sentencia preparada
            ...
            if ($sentencia_delete_historial->execute() AND $sentencia_delete_soportes->execute() AND $sentencia_delete_soportes_his->execute()) {
                // Prepara la sentencia
                $sentencia_delete_calificaciones = $enlace_db->prepare("DELETE FROM `tb_gestion_auditoria_monitoreo_calificaciones` WHERE `gcmc_monitoreo`=?");
                // Agrega variables a sentencia preparada
                ...
                if ($sentencia_delete_calificaciones->execute()) {
                    // Prepara la sentencia
                    $sentencia_delete_monitoreo = $enlace_db->prepare("DELETE FROM `tb_gestion_auditoria_monitoreo` WHERE `gcm_id`=?");
                    // Agrega variables a sentencia preparada
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group">
                            <?php if(count($resultado_registros_validar)==0 AND $_SESSION['monitoreo_registro_eliminado']!=1): ?>
