<?php
    ...
    unset($_SESSION['monitoreo_registro_eliminado_doc']);

    $titulo_header = "Monitoreos | Soportes - Editar";
    $pagina=validar_input($_GET['pagina']);
    $bandeja=validar_input(base64_decode($_GET['bandeja']));
    $filtro_permanente=validar_input($_GET['id']);
    $id_registro=validar_input(base64_decode($_GET['reg']));

    if(isset($_POST["actualizar_monitoreo_doc"])){
        ...
        foreach($_FILES["documento"]['tmp_name'] as $key => $tmp_name) {
            if ($_FILES["documento"]["name"][$key]!="") {
                ...
                $nombre_soporte_1 = $id_registro."-documento-".$_FILES['documento']['name'][$key];
                ...
                if ($_FILES['documento']["error"][$key] > 0) {
                  ...
                  /*ahora co la funcion move_uploaded_file lo guardaremos en el destino que queramos*/
                    if (move_uploaded_file($_FILES['documento']['tmp_name'][$key], $ruta_final)) {
                        ...
                        $consulta_registro_soporte_insert->bind_param('sssss', $id_registro,$nombre_soporte_1,$ruta_final,$archivo_extension, $_SESSION['usu_id']);
                        ...
                        if ($consulta_registro_soporte_insert->execute()) {
                            ...
                            //insert log eventos
                                ...
                                $log_detalle="Monitoreo Calidad | ".$id_registro." | Soporte ".$nombre_soporte_1;
        ...
        foreach($_FILES["documento_llamada"]['tmp_name'] as $key => $tmp_name) {
            if ($_FILES["documento_llamada"]["name"][$key]!="") {
                ...
                $nombre_soporte_1 = $id_registro."-llamada-".$_FILES['documento_llamada']['name'][$key];
                $ruta_final=$ruta_guardar."/".$nombre_soporte_1;
        ...
        foreach($_FILES["documento_comentario"]['tmp_name'] as $key => $tmp_name) {
            if ($_FILES["documento_comentario"]["name"][$key]!="") {
                ...
                if ($_FILES['documento_comentario']["error"][$key] > 0) {
                    $respuesta_accion = "<script type='text/javascript'>alertify.warning('¡Problemas al cargar el documento, por favor verifique e intente nuevamente!', 0);</script>";
                    $control_documento_3=0;
                } else {
                  /*ahora co la funcion move_uploaded_file lo guardaremos en el destino que queramos*/
                    if (move_uploaded_file($_FILES['documento_comentario']['tmp_name'][$key], $ruta_final)) {
                        $control_documento_3=1;
                        $id_ultimo_registro=0;
                        // Prepara la sentencia
                        $consulta_registro_soporte_insert = $enlace_db->prepare("INSERT INTO `tb_gestion_auditoria_monitoreo_historial_soportes`(`gcmhs_monitoreo`, `gcmhs_historial`, `gcmhs_nombre`, `gcmhs_ruta`, `gcmhs_extension`, `gcmhs_registro_usuario`) VALUES (?,?,?,?,?,?);");
                        ...
                        if ($consulta_registro_soporte_insert->execute()) {
                            ...
                            //insert log eventos
                                ...
                                $log_tipo="crear";
                                $log_accion="Crear registro";
                                $log_detalle="Monitoreo Calidad | ".$id_registro." | Soporte comentario ".$nombre_soporte_1;
                                $log_usuario=$_SESSION["usu_id"];
    ...
    $consulta_string_adjuntos="SELECT `gcms_id`, `gcms_monitoreo`, `gcms_nombre`, `gcms_ruta`, `gcms_extension`, `gcms_registro_usuario`, `gcms_registro_fecha` FROM `tb_gestion_auditoria_monitoreo_soportes` WHERE `gcms_monitoreo`=?";
    ...
    $consulta_string_adjuntos_his="SELECT `gcmhs_id`, `gcmhs_monitoreo`, `gcmhs_nombre`, `gcmhs_ruta`, `gcmhs_extension`, `gcmhs_registro_usuario`, `gcmhs_registro_fecha` FROM `tb_gestion_auditoria_monitoreo_historial_soportes` WHERE `gcmhs_monitoreo`=?";
...
<body>
    ...
    <div class="contenido">
        ...
        <form name="actualizar_monitoreo_doc" action="" method="POST" enctype="multipart/form-data">
        <div class="row pt-2" id="tabla_fixed">
            ...
            <div class="col-md-8">
                <div class="row">
                        ...
                        <div class="font-size-11 p-1">
                            <?php if(count($resultado_registros_adjuntos)>0): ?>
                                <?php for ($i=0; $i < count($resultado_registros_adjuntos); $i++): ?>
                                    <a href="gestion_calidad_monitoreo_ver_soporte_descargar.php?reg=<?php echo $resultado_registros_adjuntos[$i][0]; ?>&tipo=mon" target="_blank" title="Descargar"><span class="fas fa-download"></span></a> <a href="<?php echo $resultado_registros_adjuntos[$i][3]; ?>" target="_blank" title="<?php echo $resultado_registros_adjuntos[$i][2]; ?>"><span class="fas fa-external-link-alt"></span></a> <a href="gestion_calidad_monitoreo_editar_soportes_eliminar.php?doc=<?php echo base64_encode($resultado_registros_adjuntos[$i][0]); ?>&pagina=<?php echo $pagina; ?>&id=<?php echo $filtro_permanente; ?>&reg=<?php echo base64_encode($id_registro); ?>&bandeja=<?php echo base64_encode($bandeja); ?>&tipo=mon" title="Eliminar"><span class="fas fa-trash-alt color-rojo"></span></a> <?php echo $resultado_registros_adjuntos[$i][2]; ?><br>
                                <?php endfor; ?>
                            <?php else: ?>
                            ...
                            <?php endif; ?>
                            <?php if(count($resultado_registros_adjuntos_his)>0): ?>
                                <?php for ($i=0; $i < count($resultado_registros_adjuntos_his); $i++): ?>
                                        <a href="gestion_calidad_monitoreo_ver_soporte_descargar.php?reg=<?php echo $resultado_registros_adjuntos_his[$i][0]; ?>&tipo=his" target="_blank" title="Descargar"><span class="fas fa-download"></span></a> <a href="<?php echo $resultado_registros_adjuntos_his[$i][3]; ?>" target="_blank" title="<?php echo $resultado_registros_adjuntos_his[$i][2]; ?>"><span class="fas fa-external-link-alt"></span></a> 
                                    <?php if($perfil_modulo=="Administrador"): ?>
                                        <a href="gestion_calidad_monitoreo_editar_soportes_eliminar.php?doc=<?php echo base64_encode($resultado_registros_adjuntos_his[$i][0]); ?>&pagina=<?php echo $pagina; ?>&id=<?php echo $filtro_permanente; ?>&reg=<?php echo base64_encode($id_registro); ?>&bandeja=<?php echo base64_encode($bandeja); ?>&tipo=his" title="Eliminar"><span class="fas fa-trash-alt color-rojo"></span></a>
                                    <?php endif; ?>
                                    <?php echo $resultado_registros_adjuntos_his[$i][2]; ?><br>
                                <?php endfor; ?>
                            <?php endif; ?>
                        </div>
                        <?php if ($documento_cantidad!=$documento_registrados): ?>
                            <p class="alert alert-danger p-1">¡Problemas al cargar algunos soportes, por favor verifique e intente nuevamente!</p>
                        <?php endif; ?>
                    </div>
                    <div class="col-md-6">
                        <div class="form-group">
                            <label for="inputGroupFile01_label" class="m-0">Adjuntar soportes llamada <a href="#" onclick="reset_adjunto('inputGroupFile01');" class="color-rojo"><span class="fas fa-trash-alt"></span></a></label>
                            <div class="custom-file">
                                <input type="file" name="documento_llamada[]" class="custom-file-input" id="inputGroupFile01" aria-describedby="inputGroupFileAddon01" multiple>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            <label for="inputGroupFile02_label" class="m-0">Adjuntar soportes monitoreo <a href="#" onclick="reset_adjunto('inputGroupFile02');" class="color-rojo"><span class="fas fa-trash-alt"></span></a></label>
                            <div class="custom-file">
                                <input type="file" name="documento[]" class="custom-file-input" id="inputGroupFile02" aria-describedby="inputGroupFileAddon02" multiple>
                                <label class="custom-file-label" for="inputGroupFile02" id="inputGroupFile02label">Seleccione un archivo</label>
                                <p class="alert alert-warning p-1 font-size-11">*Puede seleccionar varios archivos al mismo tiempo</p>
                                <div class="col-md-12 mb-1 p-0 mt-0 font-size-11" id="soportes_seleccionados_inputGroupFile02"></div>
                            </div>
                        </div>
                    </div>
                    <?php if($perfil_modulo=="Administrador"): ?>
                    <div class="col-md-6">
                        <div class="form-group">
                            <label for="inputGroupFile02_label" class="m-0">Adjuntar soportes comentario <a href="#" onclick="reset_adjunto('inputGroupFile03');" class="color-rojo"><span class="fas fa-trash-alt"></span></a></label>
                            <div class="custom-file">
                                <input type="file" name="documento_comentario[]" class="custom-file-input" id="inputGroupFile03" aria-describedby="inputGroupFileAddon02" multiple>
                                <label class="custom-file-label" for="inputGroupFile03" id="inputGroupFile03label">Seleccione un archivo</label>
                                <p class="alert alert-warning p-1 font-size-11">*Puede seleccionar varios archivos al mismo tiempo</p>
                                <div class="col-md-12 mb-1 p-0 mt-0 font-size-11" id="soportes_seleccionados_inputGroupFile03"></div>
                            </div>
                        </div>
                    </div>
                    <?php endif; ?>
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group">
                                <button class="btn btn-success float-right ml-1" type="submit" name="actualizar_monitoreo_doc">Guardar</button>
                            <?php if(!isset($_POST["actualizar_monitoreo_doc"])): ?>
    ...
    <script type="text/javascript">
        ...
        $("#inputGroupFile03").change(function(){
            var soportes='<b>Documentos seleccionados:</b><br>';
            for (var i = 0; i < document.getElementById("inputGroupFile03").files.length; i++) {
                soportes += '<p class="alert alert-success px-1 py-0 mb-0">'+document.getElementById("inputGroupFile03").files[i].name+'</p>';
            }

            var valor_opcion = document.getElementById("inputGroupFile03").files[0].name;

            if (valor_opcion!="") {
                document.getElementById('inputGroupFile03label').innerHTML=valor_opcion.substring(0, 25)+"...";
                $("#inputGroupFile03label").addClass("color-verde");
                document.getElementById('soportes_seleccionados_inputGroupFile03').innerHTML=soportes;
            } else {
                document.getElementById('soportes_seleccionados_inputGroupFile03').innerHTML="";
            }
        });
