        ...
        if($_SESSION['registro_creado_cambio_estado']!=1){
            ...
            if ($sentencia_insert->execute()) {
                ...
                if ($documento_cantidad==$documento_registrados) {
                    ...
                    // Prepara la sentencia
                    $sentencia_delete_comentario = $enlace_db->prepare("DELETE FROM `tb_gestion_auditoria_monitoreo_historial` WHERE `gcmh_id`=?");
                    ...
                    // Prepara la sentencia
                    $sentencia_delete_soportes = $enlace_db->prepare("DELETE FROM `tb_gestion_auditoria_monitoreo_historial_soportes` WHERE `gcmhs_historial`=?");
                    ...
                    // Prepara la sentencia
                    $consulta_actualizar_reverse = $enlace_db->prepare("UPDATE `tb_gestion_auditoria_monitoreo` SET `gcm_estado`=? WHERE `gcm_id`=?");
