<?php
    ...
    // OJO: aquí depende de cómo venga $perfil_modulo desde validaciones_seguridad.php
    // Se mantiene tu lógica: si es Gestor filtra por responsable asignado.
    if (isset($perfil_modulo) && $perfil_modulo == "Gestor") {
        $filtro_perfil = " AND TMC.`gcm_auditoria_responsable`=?";
        $data_consulta[] = $_SESSION["usu_id"];
    }

    $consulta_string_transacciones = "
        SELECT
            TMC.`gcm_id`,
            ...
            TMC.`gcm_aplica_indicador`,
            TMC.`gcm_fecha_monitoreo`,
            TMC.`gcm_registro_usuario`,
            TM.`gcm_observaciones`,
            TMC.`gcm_auditoria`,
            TMC.`gcm_auditoria_responsable`
        FROM `tb_gestion_calidad_monitoreo` AS TMC
        LEFT JOIN `tb_gestion_calidad_matriz` AS TM
            ON TMC.`gcm_matriz`=TM.`gcm_id`
        LEFT JOIN `tb_administrador_usuario` AS TUR
            ON TMC.`gcm_auditoria_responsable`=TUR.`usu_id`
        LEFT JOIN `tb_gestion_auditoria_monitoreo` AS TAU
            ON TMC.`gcm_id`=TAU.`gcm_id`
        WHERE 1=1
          AND TMC.`gcm_auditoria`='Si'
          $filtro_perfil
          AND TAU.`gcm_id` IS NULL
        ORDER BY TMC.`gcm_id` ASC
    ";
        ...
        <div class="row justify-content-center">
            <div class="col-md-5 pt-2 background-blanco">
                <div class="row">
                    ...
                    <div class="col-md-12">
                        <?php if (count($resultado_registros_transacciones) > 0): ?>
                            <?php for ($i=0; $i < count($resultado_registros_transacciones); $i++): ?>
                                <a href="gestion_calidad_monitoreo_evaluacion.php?pagina=<?php echo $pagina; ?>&id=<?php echo $filtro_permanente; ?>&mon=<?php echo base64_encode($resultado_registros_transacciones[$i][0]); ?>"
                                   class="btn btn-corp-menu col-md-12 text-left mb-1">
                                    <span class="fas fa-qrcode"></span>
                                    <?php echo $resultado_registros_transacciones[$i][0]; ?>
                                    [<?php echo $resultado_registros_transacciones[$i][15]; ?>]
                                </a>
                            <?php endfor; ?>
                        <?php else: ?>
                            <p class="alert alert-warning p-1 mb-0">
                                <span class="fas fa-exclamation-triangle"></span>
                                No hay transacciones disponibles para auditar (o ya fueron auditadas).
                            </p>
                        <?php endif; ?>
                    </div>
                </div>
