...
if (!function_exists('build_file_url')) {
    function build_file_url(string $path): string
    {
        ...
        if (stripos($path, 'gestion_calidad/') === 0) {
            return $scheme . '://' . $host . '/' . $path;
        }
        return $scheme . '://' . $host . '/gestion_calidad/' . ltrim($path, '/');
    }
}
...
if (count($resultado_registros_info) <= 0) {
    ...
    <div class="contenido">
        <p class="alert alert-warning p-2">
            ...
            No se encontró información del monitoreo o no está marcado como auditoría.
        </p>
...
$id_matriz_real = (string)$resultado_registros_info[0][27];
...
$consulta_registros_matriz->bind_param("s", $id_matriz_real);
$consulta_registros_matriz->execute();
$resultado_registros_matriz = $consulta_registros_matriz->get_result()->fetch_all(MYSQLI_NUM);

$consulta_string="SELECT `gcmi_id`, `gcmi_matriz`, `gcmi_item_tipo`, `gcmi_item_consecutivo`, `gcmi_item_orden`, `gcmi_descripcion`,
`gcmi_peso`, `gcmi_calificable`, `gcmi_grupo_peso`, `gcmi_visible`, `gcmi_tipo_error`, `gcmi_grupo_id`, `gcmi_subgrupo_id`, `gcmi_item_id`,
`gcmi_subitem_id`, `gcmi_nota_maxima`, `gcmi_nota_sla`
FROM `tb_gestion_calidad_matriz_item`
WHERE `gcmi_matriz`=? ORDER BY `gcmi_item_consecutivo` ASC";

$consulta_registros = $enlace_db->prepare($consulta_string);
$consulta_registros->bind_param("s", $id_matriz_real);
$consulta_registros->execute();
$resultado_registros = $consulta_registros->get_result()->fetch_all(MYSQLI_NUM);

$consulta_string_adjuntos="SELECT `gcms_id`, `gcms_monitoreo`, `gcms_nombre`, `gcms_ruta`, `gcms_extension`, `gcms_registro_usuario`, `gcms_registro_fecha`
FROM `tb_gestion_calidad_monitoreo_soportes` WHERE `gcms_monitoreo`=?";
...
<div class="contenido">
    <div class="row" id="elemento_1">
        ...
        <div class="col-md-9 py-2">
            <a href="gestion_calidad_monitoreo_matriz.php?pagina=<?php echo h($pagina); ?>&id=<?php echo h($filtro_permanente); ?>"
               class="btn btn-corp menu float-right">
    ...
    <form name="guardar_monitoreo"
          action="gestion_calidad_monitoreo_guardar.php?pagina=<?php echo h($pagina); ?>&id=<?php echo h($filtro_permanente); ?>&mon=<?php echo h(base64_encode($id_monitoreo)); ?>"
                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-striped table-hover table-sm">
                        ...
                        <tr>
                            ...
                            <td class="align-middle py-0 font-size-11"><?php echo h($resultado_registros_info[0][0]); ?></td>
                        </tr>
                        <tr>
                            <th class="align-middle text-left py-0 font-size-11" style="width: 150px;">Matriz</th>
                            <td class="align-middle py-0 font-size-11">
                                <?php echo h($resultado_registros_matriz[0][1] ?? ''); ?><br>
                                [<?php echo h($resultado_registros_matriz[0][3] ?? ''); ?>]
                            </td>
                        </tr>
                        <tr>
                            <th class="align-middle text-left py-0 font-size-11">Fecha gestión</th>
                            <td class="align-middle py-0 font-size-11"><?php echo h($resultado_registros_info[0][2]); ?></td>
                        </tr>
                        <tr>
                            <th class="align-middle text-left py-0 font-size-11">Fecha monitoreo</th>
                            <td class="align-middle py-0 font-size-11"><?php echo h($resultado_registros_info[0][22]); ?></td>
                        </tr>
                        <tr>
                            <th class="align-middle text-left py-0 font-size-11">Tipo monitoreo</th>
                            <td class="align-middle py-0 font-size-11"><?php echo h($resultado_registros_info[0][4]); ?></td>
                        </tr>
                        <tr>
                            <th class="align-middle text-left py-0 font-size-11">Skill interacción</th>
                            <td class="align-middle py-0 font-size-11"><?php echo h($resultado_registros_info[0][5]); ?></td>
                        </tr>
                        <tr>
                            <th class="align-middle text-left py-0 font-size-11">Tipo gestión</th>
                            <td class="align-middle py-0 font-size-11"><?php echo h($resultado_registros_info[0][6]); ?></td>
                        </tr>
                        <tr>
                            <th class="align-middle text-left py-0 font-size-11">Segmento</th>
                            <td class="align-middle py-0 font-size-11"><?php echo h($resultado_registros_info[0][7]); ?></td>
                        </tr>
                        <tr>
                            <th class="align-middle text-left py-0 font-size-11">Id SIM</th>
                            <td class="align-middle py-0 font-size-11"><?php echo h($resultado_registros_info[0][8]); ?></td>
                        </tr>
                        <tr>
                            <th class="align-middle text-left py-0 font-size-11">Id/ANI</th>
                            <td class="align-middle py-0 font-size-11"><?php echo h($resultado_registros_info[0][9]); ?></td>
                        </tr>
                        </thead>
                    </table>
                </div>

                <div class="fondo-corporativo color-blanco p-1 font-size-11">
                    <span class="fas fa-paperclip"></span> Adjuntos monitoreo
                </div>
                    ...
                    <?php if(count($resultado_registros_adjuntos)>0): ?>
                        <?php for ($i=0; $i < count($resultado_registros_adjuntos); $i++): ?>
                            <?php
                                ...
                                $urlAdj    = build_file_url($rutaAdj);
                                $nombreShort = (strlen($nombreAdj)>40)? substr($nombreAdj, 0, 40).'...' : $nombreAdj;
                            ?>
                            <a href="javascript:void(0)"
                               onclick="srcChange('<?php echo h($urlAdj); ?>','<?php echo h($nombreAdj); ?>')"
                               title="<?php echo h($nombreAdj); ?>">
                        ...
                        <table class="table table-bordered table-striped table-hover table-sm">
                            ...
                            <?php for ($i=0; $i < count($resultado_registros); $i++): ?>
                                <?php if($resultado_registros[$i][9]=="Si"): ?>
                                    <tr class="<?php
                                        if($resultado_registros[$i][2]=='Grupo'){echo'matriz-grupo';}
                                        elseif($resultado_registros[$i][2]=='Sub-Grupo'){echo'matriz-grupo-sub';}
                                        elseif($resultado_registros[$i][2]=='Item'){echo'matriz-item';}
                                    ?>">
                                        <td class="align-middle">
                                            <?php if($resultado_registros[$i][2]=='Grupo' AND $resultado_registros[$i][10]=='ECU'): ?>
                                                <input type="hidden" name="peso_sla_ecuf" value="<?php echo h($resultado_registros[$i][16]); ?>">
                                            <?php endif; ?>
                                            <?php if($resultado_registros[$i][2]=='Grupo' AND $resultado_registros[$i][10]=='ENC'): ?>
                                                <input type="hidden" name="peso_sla_enc" value="<?php echo h($resultado_registros[$i][16]); ?>">
                                            <?php endif; ?>
                                            <?php if($resultado_registros[$i][2]=='Grupo' AND $resultado_registros[$i][10]=='ECN'): ?>
                                                <input type="hidden" name="peso_sla_ecn" value="<?php echo h($resultado_registros[$i][16]); ?>">
                                            <?php endif; ?>

                                            <?php if($resultado_registros[$i][7]=="Si"): ?>
                                                <input type="hidden" name="id_campos[]" value="<?php echo h($resultado_registros[$i][0]); ?>">
                                                <input type="hidden" name="grupo_peso[]" value="<?php echo h($resultado_registros[$i][8]); ?>">
                                                <input type="hidden" name="peso_nota[]" value="<?php echo h($resultado_registros[$i][6]); ?>">
                                                <input type="hidden" name="tipo_error[]" value="<?php echo h($resultado_registros[$i][10]); ?>">
                                            <?php endif; ?>
                                            <?php echo h($resultado_registros[$i][3]); ?>
                                        </td>

                                        <td class="align-middle"><?php echo h($resultado_registros[$i][5]); ?></td>
                                        <td class="align-middle text-center">
                                            <?php echo h($resultado_registros[$i][6]); ?>% <?php if($resultado_registros[$i][2]=='Grupo') { echo '/ SLA-'.h($resultado_registros[$i][16]).'%'; } ?>
                                        </td>
                                                ...
                                                <div class="form-group m-0 p-0">
                                                    <div class="form-group custom-control custom-checkbox m-0">
                                                        <input type="radio" class="custom-control-input"
                                                               id="customCheckreqsi<?php echo h($resultado_registros[$i][0]); ?>"
                                                               name="respuesta_<?php echo h($resultado_registros[$i][0]); ?>"
                                                               value="Si" checked
                                                               <?php if($_SESSION['incidencia_guardada']==1) { echo 'disabled'; } ?>
                                                               onclick="validar_comentario('Si', '<?php echo h($resultado_registros[$i][0]); ?>');" required>
                                                        <label class="custom-control-label p-0 m-0" for="customCheckreqsi<?php echo h($resultado_registros[$i][0]); ?>"></label>
                                                    </div>
                                                </div>
                                            <?php endif; ?>
                                        </td>
                                        ...
                                        <td class="align-middle text-center">
                                            <?php if($resultado_registros[$i][7]=="Si"): ?>
                                                <div class="form-group m-0 p-0">
                                                    <div class="form-group custom-control custom-checkbox m-0">
                                                        <input type="radio" class="custom-control-input"
                                                               id="customCheckreqno<?php echo h($resultado_registros[$i][0]); ?>"
                                                               name="respuesta_<?php echo h($resultado_registros[$i][0]); ?>"
                                                               value="No"
                                                               <?php if($_SESSION['incidencia_guardada']==1) { echo 'disabled'; } ?>
                                                               onclick="validar_comentario('No', '<?php echo h($resultado_registros[$i][0]); ?>');" required>
                                                        <label class="custom-control-label p-0 m-0" for="customCheckreqno<?php echo h($resultado_registros[$i][0]); ?>"></label>
                                                    </div>
                                                </div>
                                            <?php endif; ?>
                                        </td>
                                        ...
                                        <td class="align-middle text-center">
                                            <?php if($resultado_registros[$i][7]=="Si"): ?>
                                                <input type="text" class="form-control form-control-sm d-none"
                                                       name="comentario_<?php echo h($resultado_registros[$i][0]); ?>"
                                                       id="comentario_<?php echo h($resultado_registros[$i][0]); ?>"
                                                       maxlength="2000" required disabled value="">
                                            <?php endif; ?>
                                        </td>
                                    </tr>
                                <?php endif; ?>
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            <textarea class="form-control form-control-sm" name="observaciones" id="observaciones"><?php
                                if(isset($_SESSION["mon_informacion"]["observaciones"])){
                                    echo h($_SESSION["mon_informacion"]["observaciones"]);
                                }
                            ?></textarea>
                        </div>
                    </div>
            ...
            <div class="col-md-12">
                <div class="form-group">
                    <?php if($_SESSION['registro_creado']==1): ?>
                        <a href="<?php echo h($ruta_cancelar_finalizar); ?>" class="btn btn-dark">Finalizar</a>
                    <?php else: ?>
...
<script type="text/javascript">
    ...
    function tabla_fixed(){
        ...
        if (!tabla_fixed || !elemento || !elemento_1) return;
