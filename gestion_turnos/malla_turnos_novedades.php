...
// ini_set('display_errors', '1');
    ...
    $titulo_header = "Gestión Turnos | Novedades";
    ...
    if ($filtro_operacion!='Todas') {
        $filtro_usuario_operacion='AND (TUT.`usu_campania`=?)';
    ...
    if ($perfil_modulo=="Administrador" OR $perfil_modulo=="Gestor" OR $perfil_modulo=="Formador" OR $perfil_modulo=="Visitante" OR $perfil_modulo=="Cliente" OR $perfil_modulo=="Usuario") {
        ...
        $filtro_supervisor=" AND (TUT.`usu_supervisor`=?)";
        array_push($data_consulta_supervisor, $_SESSION["usu_id"]);
        array_push($data_consulta, $_SESSION["usu_id"]);
    }

    $consulta_string_areas="SELECT DISTINCT `usu_campania`, TC.`ac_nombre_campania` FROM `tb_administrador_usuario` LEFT JOIN `tb_administrador_campania` AS TC ON `tb_administrador_usuario`.`usu_campania`=TC.`ac_id` WHERE `usu_estado`<>'Retirado' ".str_replace('TUT.', '', $filtro_supervisor)." ORDER BY TC.`ac_nombre_campania` ASC";
    ...
    if ($filtro_permanente!="null" AND $filtro_permanente!="") {
        $filtro_buscar="AND (`ctm_usuario` LIKE ? OR TUT.`usu_nombres_apellidos` LIKE ? OR `ctmn_tipo` LIKE ? OR `ctmn_fecha_inicio` LIKE ? OR `ctmn_fecha_fin` LIKE ? OR `ctmn_registro_usuario` LIKE ? OR TUR.`usu_nombres_apellidos` LIKE ? OR `ctmn_registro_fecha` LIKE ? OR TC.`ac_nombre_campania` LIKE ?)";
    ...
    $consulta_contar_string="SELECT COUNT(`ctmn_id`) FROM `tb_control_turno_malla_novedades` LEFT JOIN `tb_administrador_usuario` AS TUT ON `tb_control_turno_malla_novedades`.`ctm_usuario`=TUT.`usu_id` LEFT JOIN `tb_administrador_usuario` AS TUR ON `tb_control_turno_malla_novedades`.`ctmn_registro_usuario`=TUR.`usu_id` LEFT JOIN `tb_administrador_campania` AS TC ON TUT.`usu_campania`=TC.`ac_id` WHERE `ctmn_registro_fecha` LIKE ? ".$filtro_usuario_operacion." ".$filtro_supervisor." ".$filtro_buscar."";
    ...
    if (!isset($_GET['pagina']) || ($pagina>$numero_paginas AND $numero_paginas>0) || $pagina<=0) {
        header('Location:malla_turnos_novedades.php?pagina=1&id=null');
    }
    ...
    <div class="contenido">
        <div class="row" id="elemento_1">
                ...
                <form name="filtrado" action="" method="POST">
                    <div class="input-group">
                        ...
                        <span class="input-group-btn">
                            ...
                            <a href="malla_turnos_novedades.php?pagina=1&id=null&fechainicio=<?php echo base64_encode(date('Y-m'));?>&operacion=<?php echo base64_encode('Todas'); ?>" class="btn btn-corp"><span class="fas fa-sync-alt"></span></a>
                        </span>
                    </div>
                </form>
            </div>
            <div class="col-md-5 py-2">
                ...
                <a href="malla_turnos_novedades_crear.php?pagina=1&id=null&fechainicio=<?php echo base64_encode(date('Y-m'));?>&operacion=<?php echo base64_encode('Todas'); ?>" class="btn btn-corp menu float-right"><div class="float-left"><span class="fas fa-plus"></span></div></a>
            </div>
        </div>
                    ...
                    <div class="col-md-12 float-left">
                        <nav aria-label="Paginación" class="paginacion">
                            <ul class="pagination justify-content-end">
                                <li class="page-item <?php echo $pagina<=1 ? 'disabled':'' ?>"><a class="page-link" href="malla_turnos_novedades.php?pagina=1&id=<?php echo $filtro_permanente; ?>&fechainicio=<?php echo base64_encode($FechaInicio); ?>&operacion=<?php echo base64_encode($filtro_operacion); ?>"><span class="fas fa-angle-double-left"></span></a></li>
                                <li class="page-item <?php echo $pagina<=1 ? 'disabled':'' ?>"><a class="page-link" href="malla_turnos_novedades.php?pagina=<?php echo $pagina-1; ?>&id=<?php echo $filtro_permanente; ?>&fechainicio=<?php echo base64_encode($FechaInicio); ?>&operacion=<?php echo base64_encode($filtro_operacion); ?>"><span class="fas fa-angle-left"></span></a></li>
                                <?php
                                ...
                                <?php for ($i=$pagina_inicio; $i <= $pagina_fin; $i++): ?>
                                    <li class="page-item <?php echo $pagina==$i ? 'active':'' ?>"><a class="page-link" href="malla_turnos_novedades.php?pagina=<?php echo $i; ?>&id=<?php echo $filtro_permanente; ?>&fechainicio=<?php echo base64_encode($FechaInicio); ?>&operacion=<?php echo base64_encode($filtro_operacion); ?>"><?php echo $i; ?></a></li>
                                <?php endfor; ?>
                                <li class="page-item <?php echo $pagina>=$numero_paginas ? 'disabled':'' ?>"><a class="page-link" href="malla_turnos_novedades.php?pagina=<?php echo $pagina+1; ?>&id=<?php echo $filtro_permanente; ?>&fechainicio=<?php echo base64_encode($FechaInicio); ?>&operacion=<?php echo base64_encode($filtro_operacion); ?>"><span class="fas fa-angle-right"></a></li>
                                <li class="page-item <?php echo $pagina>=$numero_paginas ? 'disabled':'' ?>"><a class="page-link" href="malla_turnos_novedades.php?pagina=<?php echo $numero_paginas; ?>&id=<?php echo $filtro_permanente; ?>&fechainicio=<?php echo base64_encode($FechaInicio); ?>&operacion=<?php echo base64_encode($filtro_operacion); ?>"><span class="fas fa-angle-double-right"></a></li>
                            </ul>
                        </nav>
                    </div>
                        ...
                        <table class="table table-bordered table-striped table-hover table-sm">
                            ...
                            <tbody>    
                                ...
                                <tr>
                                    ...
                                    <td class="align-middle"><?php echo $array_convenciones[$resultado_registros[$i][3]]; ?></td>
