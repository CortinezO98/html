...
// ini_set('display_errors', '1');
    ...
    $titulo_header = "Gestión Turnos | Cambio Turno";
    $pagina=validar_input($_GET['pagina']);

    unset($_SESSION['mturno_cambioturno_editar']);
    ...
    if ($filtro_operacion!='Todas') {
        $filtro_usuario_operacion='AND (TUO.`usu_campania`=?)';
        array_push($data_consulta, $filtro_operacion);
    }
    ...
    if ($perfil_modulo=="Administrador" OR $perfil_modulo=="Gestor" OR $perfil_modulo=="Formador" OR $perfil_modulo=="Visitante" OR $perfil_modulo=="Cliente" OR $perfil_modulo=="Usuario") {
        ...
        $filtro_supervisor=" AND (TUO.`usu_supervisor`=?)";
        array_push($data_consulta_supervisor, $_SESSION["usu_id"]);
        array_push($data_consulta, $_SESSION["usu_id"]);
    }

    $consulta_string_areas="SELECT DISTINCT `usu_campania`, TC.`ac_nombre_campania` FROM `tb_administrador_usuario` LEFT JOIN `tb_administrador_campania` AS TC ON `tb_administrador_usuario`.`usu_campania`=TC.`ac_id` WHERE `usu_estado`<>'Retirado' ".str_replace('TUO.', '', $filtro_supervisor)." ORDER BY TC.`ac_nombre_campania` ASC";
    ...
    if ($filtro_permanente!="null" AND $filtro_permanente!="") {
        $filtro_buscar="AND (`cmtct_usuario_origen` LIKE ? OR TUO.`usu_nombres_apellidos` LIKE ? OR TCUO.`ac_nombre_campania` LIKE ? OR `cmtct_uo_turno_anterior_inicio` LIKE ? OR `cmtct_uo_turno_anterior_fin` LIKE ? OR `cmtct_uo_turno_nuevo_inicio` LIKE ? OR `cmtct_uo_turno_nuevo_fin` LIKE ? OR `cmtct_usuario_destino` LIKE ? OR TUD.`usu_nombres_apellidos` LIKE ? OR TCUD.`ac_nombre_campania` LIKE ? OR `cmtct_ud_turno_anterior_inicio` LIKE ? OR `cmtct_ud_turno_anterior_fin` LIKE ? OR `cmtct_ud_turno_nuevo_inicio` LIKE ? OR `cmtct_ud_turno_nuevo_fin` LIKE ? OR `cmtct_responsable` LIKE ? OR TR.`usu_nombres_apellidos` LIKE ? OR `cmtct_estado` LIKE ? OR `cmtct_observaciones` LIKE ?)";
    ...
    $consulta_contar_string="SELECT COUNT(`cmtct_id`) FROM `tb_control_turno_malla_cambio_turno` LEFT JOIN `tb_administrador_usuario` AS TUO ON `tb_control_turno_malla_cambio_turno`.`cmtct_usuario_origen`=TUO.`usu_id` LEFT JOIN `tb_administrador_usuario` AS TUD ON `tb_control_turno_malla_cambio_turno`.`cmtct_usuario_destino`=TUD.`usu_id` LEFT JOIN `tb_administrador_usuario` AS TR ON `tb_control_turno_malla_cambio_turno`.`cmtct_responsable`=TR.`usu_id` LEFT JOIN `tb_administrador_campania` AS TCUO ON TUO.`usu_campania`=TCUO.`ac_id` LEFT JOIN `tb_administrador_campania` AS TCUD ON TUD.`usu_campania`=TCUD.`ac_id` WHERE `cmtct_registro_fecha` LIKE ? ".$filtro_usuario_operacion." ".$filtro_supervisor." ".$filtro_buscar."";
    ...
    if (!isset($_GET['pagina']) || ($pagina>$numero_paginas AND $numero_paginas>0) || $pagina<=0) {
        header('Location:malla_turnos_cambio.php?pagina=1&id=null');
    }
...
<body onresize="tabla_fixed();" onload="tabla_fixed();">
                ...
                <form name="filtrado" action="" method="POST">
                    <div class="input-group">
                        ...
                        <span class="input-group-btn">
                            ...
                            <a href="malla_turnos_cambio.php?pagina=1&id=null&fechainicio=<?php echo base64_encode(date('Y-m'));?>&operacion=<?php echo base64_encode('Todas'); ?>" class="btn btn-corp"><span class="fas fa-sync-alt"></span></a>
                        </span>
                    </div>
                </form>
            </div>
                    ...
                    <div class="col-md-12 float-left">
                        <nav aria-label="Paginación" class="paginacion">
                            <ul class="pagination justify-content-end">
                                <li class="page-item <?php echo $pagina<=1 ? 'disabled':'' ?>"><a class="page-link" href="malla_turnos_cambio.php?pagina=1&id=<?php echo $filtro_permanente; ?>&fechainicio=<?php echo base64_encode($FechaInicio); ?>&operacion=<?php echo base64_encode($filtro_operacion); ?>"><span class="fas fa-angle-double-left"></span></a></li>
                                <li class="page-item <?php echo $pagina<=1 ? 'disabled':'' ?>"><a class="page-link" href="malla_turnos_cambio.php?pagina=<?php echo $pagina-1; ?>&id=<?php echo $filtro_permanente; ?>&fechainicio=<?php echo base64_encode($FechaInicio); ?>&operacion=<?php echo base64_encode($filtro_operacion); ?>"><span class="fas fa-angle-left"></span></a></li>
                                <?php
                                ...
                                <?php for ($i=$pagina_inicio; $i <= $pagina_fin; $i++): ?>
                                    <li class="page-item <?php echo $pagina==$i ? 'active':'' ?>"><a class="page-link" href="malla_turnos_cambio.php?pagina=<?php echo $i; ?>&id=<?php echo $filtro_permanente; ?>&fechainicio=<?php echo base64_encode($FechaInicio); ?>&operacion=<?php echo base64_encode($filtro_operacion); ?>"><?php echo $i; ?></a></li>
                                <?php endfor; ?>
                                <li class="page-item <?php echo $pagina>=$numero_paginas ? 'disabled':'' ?>"><a class="page-link" href="malla_turnos_cambio.php?pagina=<?php echo $pagina+1; ?>&id=<?php echo $filtro_permanente; ?>&fechainicio=<?php echo base64_encode($FechaInicio); ?>&operacion=<?php echo base64_encode($filtro_operacion); ?>"><span class="fas fa-angle-right"></a></li>
                                <li class="page-item <?php echo $pagina>=$numero_paginas ? 'disabled':'' ?>"><a class="page-link" href="malla_turnos_cambio.php?pagina=<?php echo $numero_paginas; ?>&id=<?php echo $filtro_permanente; ?>&fechainicio=<?php echo base64_encode($FechaInicio); ?>&operacion=<?php echo base64_encode($filtro_operacion); ?>"><span class="fas fa-angle-double-right"></a></li>
                            </ul>
                        </nav>
                    </div>
                        ...
                        <table class="table table-bordered table-striped table-hover table-sm">
                            <thead>
                                <tr>
                                    ...
                                    <th class="align-middle">Doc. Solicitante</th>
                                    <th class="align-middle">Solicitante</th>
                                    <th class="align-middle">Campaña</th>
                                    <th class="align-middle">Turno Anterior</th>
                                    <th class="align-middle">Turno Nuevo</th>
                                    <th class="align-middle">Doc. Solicitado</th>
                                    <th class="align-middle">Solicitado</th>
                                    <th class="align-middle">Turno Anterior</th>
                                    <th class="align-middle">Turno Nuevo</th>
                            ...
                            <tbody>    
                                ...
                                <tr>
                                    <td class="align-middle text-center">
                                        <?php if($resultado_registros[$i][17]!="Cancelado" AND $resultado_registros[$i][17]!="Rechazado"): ?>
                                            <a href="malla_turnos_cambio_editar.php?pagina=<?php echo $pagina; ?>&id=<?php echo $filtro_permanente; ?>&fechainicio=<?php echo base64_encode($FechaInicio);?>&operacion=<?php echo base64_encode($filtro_operacion); ?>&reg=<?php echo base64_encode($resultado_registros[$i][0]); ?>" class="btn btn-primary btn-sm btn-width mb-1" title="Editar"><span class="fas fa-pen"></span></a>
                                        <?php endif; ?>
                                    </td>
                                    <td class="align-middle"><?php echo $resultado_registros[$i][17]; ?></td>
                                    <td class="align-middle"><?php echo date('Y-m-d', strtotime($resultado_registros[$i][4])); ?></td>
                                    ...
                                    <td class="align-middle"><?php echo date('H:i', strtotime($resultado_registros[$i][4])).'-'.date('H:i', strtotime($resultado_registros[$i][5])); ?></td>
                                    <td class="align-middle"><?php echo date('H:i', strtotime($resultado_registros[$i][6])).'-'.date('H:i', strtotime($resultado_registros[$i][7])); ?></td>
                                    <td class="align-middle"><?php echo $resultado_registros[$i][8]; ?></td>
                                    <td class="align-middle"><?php echo $resultado_registros[$i][9]; ?></td>
                                    <td class="align-middle"><?php echo date('H:i', strtotime($resultado_registros[$i][11])).'-'.date('H:i', strtotime($resultado_registros[$i][12])); ?></td>
                                    <td class="align-middle"><?php echo date('H:i', strtotime($resultado_registros[$i][13])).'-'.date('H:i', strtotime($resultado_registros[$i][14])); ?></td>
                                    <td class="align-middle"><?php echo $resultado_registros[$i][16]; ?></td>
                                    <td class="align-middle"><?php echo $resultado_registros[$i][18]; ?></td>
    ...
    <?php
        ...
        include("malla_turnos_cambio_reporte.php");
    ?>
