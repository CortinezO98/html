    ...
    if(isset($_SESSION["usu_id"]) AND $_SESSION['usu_inicio_sesion']!=0){
    	...
    	if(isset($_POST["form_recovery"])){
            ...
            if ($captcha_original == sha1($captcha_validacion_recovery)) {
                $consulta_string = "SELECT `usu_id`, `usu_acceso`, `usu_contrasena`, `usu_nombres_apellidos`, `usu_correo_corporativo`, `usu_estado`, `usu_inicio_sesion`, `usu_campania` FROM `tb_administrador_usuario` WHERE `usu_id`=? AND `usu_correo_corporativo`=?";
                ...
                if (count($resultado_registros)>0) {
                	if($resultado_registros[0][5]=='Activo'){
                        ...
                        if ($sentencia_insert_token->execute()) {
				            ...
				            for ($i=0; $i < 10; $i++) {
				                $consulta_notificacion = mysqli_query($enlace_db, "INSERT INTO `tb_notificaciones_central`(`nc_id_modulo`, `nc_prioridad`, `nc_id_set_from`, `nc_address`, `nc_cc`, `nc_bcc`, `nc_reply_to`, `nc_subject`, `nc_body`, `nc_embeddedimage_ruta`, `nc_embeddedimage_nombre`, `nc_embeddedimage_tipo`, `nc_intentos`, `nc_eliminar`, `nc_estado_envio`, `nc_fecha_envio`, `nc_usuario_registro`) VALUES ('1', 'Alta','".$nc_id_set_from."','".$nc_address."','".$nc_cc."','".$nc_bcc."','','".$nc_subject."','".$nc_body."','".$nc_embeddedimage_ruta."','".$nc_embeddedimage_nombre."','".$nc_embeddedimage_tipo."', '".$nc_intentos."','".$nc_eliminar."','".$nc_estado_envio."','".$nc_fecha_envio."','".$nc_usuario_registro."');");
