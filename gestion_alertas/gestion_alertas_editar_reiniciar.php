<?php
    ...
    $titulo_header = "Gestión Alertas | Reiniciar validación";
    ...
    if(isset($_POST["guardar_registro"])){
        ...
        $consulta_actualizar = $enlace_db->prepare("UPDATE `tb_gestion_alertas` SET `ga_estado`=? WHERE `ga_id`=?");

        // Agrega variables a sentencia preparada
        $consulta_actualizar->bind_param('ss', $estado_final, $id_registro);
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            <label for="afecta_lt">Nuevo estado</label>
                            <select class="form-control form-control-sm" name="estado_final" id="estado_final" required>
                              <option value="">Seleccione</option>
                              <option value="Pendiente" <?php if($resultado_registros[0][3]=="Pendiente"){ echo "selected"; } ?>>Pendiente</option>
                            </select>
                        </div>
                    </div>
