<?php
    ...
    $titulo_header = "Gestión Alertas - Configuración | Centro Zonal";
    ...
    if ($filtro_permanente!="null" AND $filtro_permanente!="") {
        $filtro_buscar="AND (`gercz_regional` LIKE ? OR `gercz_centro_zonal` LIKE ? OR `gercz_correos` LIKE ? OR `gercz_registro_fecha` LIKE ? OR TR.`gere_regional` LIKE ? OR TR.`gere_id_mapa` LIKE ?)";
    ...
    $consulta_contar_string="SELECT COUNT(`gercz_id`) FROM `tb_gestion_encuesta_regional_czonal` LEFT JOIN `tb_gestion_encuesta_regional` AS TR ON `tb_gestion_encuesta_regional_czonal`.`gercz_regional`=TR.`gere_id` WHERE 1=1 ".$filtro_buscar."";
    ...
    if (!isset($_GET['pagina']) || ($pagina>$numero_paginas AND $numero_paginas>0) || $pagina<=0) {
        header('Location:gestion_alertas_czonales.php?pagina=1&id=null');
    }
                ...
                <form name="filtrado" action="" method="POST">
                    <div class="input-group">
                      ...
                      <span class="input-group-btn">
                        ...
                        <a href="gestion_alertas_czonales.php?pagina=1&id=null" class="btn btn-corp"><span class="fas fa-sync-alt"></span></a>
                      </span>
                    </div>
                </form>
            </div>
                    ...
                    <div class="col-md-12 float-left">
                        <nav aria-label="Paginación" class="paginacion">
                            <ul class="pagination justify-content-end">
                                <li class="page-item <?php echo $pagina<=1 ? 'disabled':'' ?>"><a class="page-link" href="gestion_alertas_czonales.php?pagina=1&id=<?php echo $filtro_permanente; ?>"><span class="fas fa-angle-double-left"></span></a></li>
                                <li class="page-item <?php echo $pagina<=1 ? 'disabled':'' ?>"><a class="page-link" href="gestion_alertas_czonales.php?pagina=<?php echo $pagina-1; ?>&id=<?php echo $filtro_permanente; ?>"><span class="fas fa-angle-left"></span></a></li>
                                <?php
                                ...
                                <?php for ($i=$pagina_inicio; $i <= $pagina_fin; $i++): ?>
                                    <li class="page-item <?php echo $pagina==$i ? 'active':'' ?>"><a class="page-link" href="gestion_alertas_czonales.php?pagina=<?php echo $i; ?>&id=<?php echo $filtro_permanente; ?>"><?php echo $i; ?></a></li>
                                <?php endfor; ?>
                                <li class="page-item <?php echo $pagina>=$numero_paginas ? 'disabled':'' ?>"><a class="page-link" href="gestion_alertas_czonales.php?pagina=<?php echo $pagina+1; ?>&id=<?php echo $filtro_permanente; ?>"><span class="fas fa-angle-right"></a></li>
                                <li class="page-item <?php echo $pagina>=$numero_paginas ? 'disabled':'' ?>"><a class="page-link" href="gestion_alertas_czonales.php?pagina=<?php echo $numero_paginas; ?>&id=<?php echo $filtro_permanente; ?>"><span class="fas fa-angle-double-right"></a></li>
                            </ul>
                        </nav>
                    </div>
                                ...
                                <tr>
                                    <td class="align-middle text-center">
                                        <?php if($perfil_modulo=="Administrador"): ?>
                                            <a href="gestion_alertas_czonales_editar.php?pagina=<?php echo $pagina; ?>&id=<?php echo $filtro_permanente; ?>&reg=<?php echo base64_encode($resultado_registros[$i][0]); ?>" class="btn btn-primary btn-sm btn-width" title="Editar"><span class="fas fa-pen"></span></a>
                                        <?php endif; ?>
                                    </td>
                                    ...
                                    <td class="align-middle"><?php echo (strlen($resultado_registros[$i][7])>80) ? substr($resultado_registros[$i][7], 0, 80)."..." : $resultado_registros[$i][7]; ?></td>
                                </tr>
                                <?php
                                    }
                                ?>
