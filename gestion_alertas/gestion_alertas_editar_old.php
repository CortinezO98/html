<?php
    ...
    if(isset($_POST["guardar_registro"])){
        ...
        if($_SESSION['gestion_alerta_valida_n1']!=1){
            ...
            $consulta_actualizar = $enlace_db->prepare("UPDATE `tb_gestion_alertas` SET `ga_estado`=?,`ga_afecta_linea_tecnica`=?,`ga_fecha_remite_agente_especializado`=?, `ga_fecha_cierre`=?,`ga_revisa_n1_usuario`=?,`ga_revisa_n1_fecha`=?, `ga_observaciones_n1`=? WHERE `ga_id`=?");

            // Agrega variables a sentencia preparada
            $consulta_actualizar->bind_param('ssssssss', $estado_final, $afecta_lt, $fecha_escala, $fecha_cierre, $_SESSION['usu_id'], date('Y-m-d H:i:s'), $observaciones, $id_registro);
            ...
            if (comprobarSentencia($enlace_db->info)) {
                ...
                $consulta_string_validar_notificacion="SELECT `ga_id`, `ga_sim_asociado`, `ga_sim_qr_encuesta`, `ga_estado`, `ga_fecha_alerta`, `ga_fecha_atencion_ciudadano`, `ga_regional`, `ga_centro_zonal`, `ga_peticionario_id`, `ga_peticionario_nombre`, `ga_peticionario_telefono`, `ga_categoria_afectacion`, `ga_descripcion_alerta`, `ga_afecta_linea_tecnica`, `ga_fecha_remite_agente_especializado`, `ga_fecha_require_regional`, `ga_fecha_respuesta_regional`, `ga_concepto_tecnico_pmejora`, `ga_decision_caso`, `ga_snc_ac`, `ga_registro_usuario`, `ga_registro_fecha`, TR.`gere_regional`, TCZ.`gercz_centro_zonal`, TU.`usu_nombres_apellidos`, TU.`usu_correo_corporativo` FROM `tb_gestion_alertas` LEFT JOIN `tb_administrador_usuario` AS TU ON `tb_gestion_alertas`.`ga_registro_usuario`=TU.`usu_id` LEFT JOIN `tb_gestion_encuesta_regional` AS TR ON `tb_gestion_alertas`.`ga_regional`=TR.`gere_id` LEFT JOIN `tb_gestion_encuesta_regional_czonal` AS TCZ ON `tb_gestion_alertas`.`ga_centro_zonal`=TCZ.`gercz_id` WHERE `ga_id`=?";
                ...
                if ($afecta_lt=="No") {
                    ...
                    /*SE CONFIGURAN PARÁMETROS A REGISTRAR EN SISTEMA DE NOTIFICACIÓN*/
                        ...
                        /*SE ESTRUCTURA COTENIDO DE CORREO*/
                        $consulta_string_destinatarios="SELECT `gad_id`, `gad_filtro`, `gad_correos_to`, `gad_correos_cc` FROM `tb_gestion_alertas_destinatarios` WHERE `gad_filtro`='noafectalt_nivel1'";
                        ...
                        $nc_address=$resultado_registros_validar_notificacion[0][25]."|".$resultado_registros_validar_notificacion[0][25].";";
                    ...
                    /*SE CONFIGURAN PARÁMETROS A REGISTRAR EN SISTEMA DE NOTIFICACIÓN*/
                        ...
                        /*SE ESTRUCTURA COTENIDO DE CORREO*/
                        $consulta_string_destinatarios="SELECT `gad_id`, `gad_filtro`, `gad_correos_to`, `gad_correos_cc` FROM `tb_gestion_alertas_destinatarios` WHERE `gad_filtro`='afectalt_nivel1'";
                        $consulta_registros_destinatarios = $enlace_db->prepare($consulta_string_destinatarios);
                        $consulta_registros_destinatarios->execute();
                        $resultado_registros_destinatarios = $consulta_registros_destinatarios->get_result()->fetch_all(MYSQLI_NUM);
                ...
                <div class="row">
                    ...
                    <div class="col-md-12 <?php if($_SESSION['gestion_alerta_valida_n1']!=1) { echo 'd-none'; } ?>" id="div_contexto">
                        <div class="form-group">
                          ...
                          <textarea class="form-control form-control-sm" name="contexto" id="contexto" rows="5" <?php if($_SESSION['gestion_alerta_valida_n1']==1) { echo 'readonly'; } ?> required><?php echo $resultado_registros[0][12]; ?></textarea>
                        </div>
                    </div>
                    <div class="col-md-12 <?php if($_SESSION['gestion_alerta_valida_n1']!=1) { echo 'd-none'; } ?>" id="div_observaciones">
                        <div class="form-group">
                          ...
                          <textarea class="form-control form-control-sm" name="observaciones" id="observaciones" rows="5" <?php if($_SESSION['gestion_alerta_valida_n1']==1) { echo 'readonly'; } ?> required><?php if(isset($_POST["guardar_registro"])){ echo $observaciones; } ?></textarea>
                        </div>
                    </div>
                </div>
                <div class="row">
