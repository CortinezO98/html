<?php
    ...
    $titulo_header = "Encuestas | Reasignar - Usuarios";

    if(isset($_POST["guardar_registro"])){
        unset($_SESSION['usuarios_reasignacion']);
        $_SESSION['usuarios_reasignacion']=$_POST['usuarios'];

        if (count($_SESSION['usuarios_reasignacion'])>0) {
            header('Location: gestion_encuestas_base_asignar.php');
        } else {
    ...
    if (!isset($_SESSION['usuarios_reasignacion'])) {
        $_SESSION['usuarios_reasignacion']=array();
    }
        ...
        <form name="guardar_registro" action="" method="POST" enctype="multipart/form-data">
            <div class="row justify-content-center">
                <div class="col-md-6">
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <p class="alert alert-warning p-1">Seleccione los usuarios para reasignación de radicados</p>
                            <?php for ($i=0; $i < count($resultado_registros_usuarios); $i++): ?>
                            <div class="form-group custom-control custom-checkbox m-0">
                                <input type="checkbox" class="custom-control-input" id="customCheck_<?php echo $resultado_registros_usuarios[$i][0]; ?>" name="usuarios[]" value="<?php echo $resultado_registros_usuarios[$i][0]; ?>" <?php if(in_array($resultado_registros_usuarios[$i][0], $_SESSION['usuarios_reasignacion'])){ echo "checked"; } ?>>
